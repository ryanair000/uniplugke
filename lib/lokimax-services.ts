...
type LokimaxServiceSource = { id: string; service_name: string };
type ServiceProfile = {
  name: string;
...
export function buildLokimaxCatalog(sources: LokimaxServiceSource[], curatedServices: CatalogService[]) {
  const curatedBySource = new Map<string, CatalogService>();
  curatedServices.forEach((service) => curatedBySource.set(normalizeLokimaxServiceName(service.name), service));
  const netflix = curatedServices.find((service) => service.slug === "netflix-premium");
  const microsoft = curatedServices.find((service) => service.slug === "microsoft-365");
  if (netflix) curatedBySource.set("netflix", netflix);
  if (microsoft) curatedBySource.set("office365", microsoft);

  const seen = new Set<string>();
  return sources.flatMap((source) => {
    const key = normalizeLokimaxServiceName(source.service_name);
    const serviceProfile = profiles[key];
    const dedupeKey = serviceProfile ? slugify(serviceProfile.name) : key;
    if (!key || seen.has(dedupeKey)) return [];
    seen.add(dedupeKey);

    const curated = curatedBySource.get(key);
    if (curated) return [curated];

    const name = serviceProfile?.name ?? source.service_name.trim();
