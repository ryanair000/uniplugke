...
export async function POST(request: Request) {
  ...
  if (body.termsAcknowledged !== true) {
    return NextResponse.json({ error: "Review and acknowledge the material licence terms before payment" }, { status: 400 });
  }
  ...
  const { data: order, error } = await admin.from("uniplug_key_orders").insert({
    ...
    licence_term: product.durationLabel,
    amount_kes: product.priceKes,
    customer_email: email,
    customer_phone: phone,
    terms_version: KEY_TERMS_VERSION,
    payment_disclosure: keyProductPaymentDisclosure(product),
    end_of_term_disclosure: keyProductEndOfTermDisclosure(product),
    material_terms_acknowledged_at: new Date().toISOString()
  }).select("id").single();
  ...
  const response = await fetch("https://api.paystack.co/transaction/initialize", {
    ...
    body: JSON.stringify({
      ...
      metadata: {
        order_type: "software_key",
        key_order_id: order.id,
        product: product.slug,
        product_name: product.name,
        licence_term: product.durationLabel,
        payment_terms: keyProductPaymentDisclosure(product),
        end_of_term_terms: keyProductEndOfTermDisclosure(product),
        material_terms_acknowledged: true,
        terms_version: KEY_TERMS_VERSION
      }
    }), cache: "no-store"
  });
