  ...
  return (
    <section className="wallet-page">
      ...
      <div className="wallet-support-grid">
        <section className="wallet-card"><div className="wallet-card-heading"><div><p className="wallet-kicker">New request</p><h2>Create a ticket</h2></div><span className="wallet-support-icon" aria-hidden="true">?</span></div><form action={createSupportTicket} className="wallet-ticket-form"><input type="hidden" name="returnTo" value="/dashboard/support"/><label>Subject<input name="subject" minLength={3} maxLength={120} required defaultValue={renewal && service ? `Renew ${service}` : service ? `${service} support` : ""} placeholder="Briefly describe the issue"/></label><label>Message<textarea name="message" minLength={10} maxLength={2000} required rows={7} defaultValue={renewal && service ? `I would like to renew my ${service} service. Please share the next payment step.` : ""} placeholder="Tell us what happened and what you already tried."/></label><p>Never include passwords, verification codes, or payment credentials.</p><button className="button wallet-primary-button" type="submit">Create ticket</button></form></section>
